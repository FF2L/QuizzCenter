...
import { CauHoi } from "../../../common/model";

interface QuestionDetailDialogProps {
  open: boolean;
  onClose: () => void;
  questionDetail: CauHoi | null; // object câu hỏi
}

const QuestionDetailDialog = ({ open, onClose, questionDetail }: QuestionDetailDialogProps) => {
  if (!questionDetail) return null;
  console.log(questionDetail.__dapAn__); // <-- thêm dòng này
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers>
        <Stack spacing={2}>
          <Typography variant="h6">Tên hiển thị: {questionDetail.tenHienThi}</Typography>
          <Typography variant="body1">Nội dung: {questionDetail.noiDungCauHoi}</Typography>
          <Typography variant="body2">Loại câu hỏi: {questionDetail.loaiCauHoi}</Typography>
          <Typography variant="body2">Độ khó: {questionDetail.doKho}</Typography>

          <Box>
            <Typography variant="subtitle1" sx={{ mt: 2, mb: 1 }}>Các đáp án:</Typography>
            <Stack spacing={1}>
              {questionDetail.__dapAn__?.map((dapAn) => (
                <Box
                  key={dapAn.id}
                  sx={{
                    display: "flex",
